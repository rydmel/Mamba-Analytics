...
MADE_SHOT_SENTENCES = [
    ...
    ("{} in the {} half, a {} shot for {} points swung the momentum in {}'s favor.  ",
...
MISSED_SHOT_SENTENCES = [
    ...
    ("A {}-point shot attempt by {} in the {} quarter led to zero points and a significant swing in momentum for his opponents.  ",
...
def generate_recap_text(game_object):
    ...
    selected_plays = select_momentum_shifting_plays(game_object)
...
def select_momentum_shifting_plays(game_object):
    if game_object.momentum_df.empty:
    ...
    for row in top_five_plays.iterrows():
        if row[0] + 1 in selected_plays or row[0] - 1 in selected_plays:
            ...
            selected_plays.append(row[0])

    return selected_plays


def generate_text_for_a_play(selected_play):
    # Determine if rebound, make or shot
    # sample from proper list